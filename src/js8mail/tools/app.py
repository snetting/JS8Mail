...
function markLiveGraphNodes(){let svg=document.querySelector('#live-graph svg');if(!svg)return;svg.querySelectorAll('circle').forEach(circle=>{circle.setAttribute('stroke','#17823b');circle.setAttribute('stroke-width','3');circle.setAttribute('fill','#d8f3dc')})}new MutationObserver(markLiveGraphNodes).observe(document.getElementById('live-graph'),{childList:true,subtree:true});
