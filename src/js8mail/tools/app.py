...
</style><h1>JS8Mail</h1><p>Resilient radio mail for reliable offline communication · by OH3SPN <button onclick="useStation('OH3SPN')">Compose to OH3SPN</button></p><section><div id=status>Loading…</div><div id=radio-state class='pill radio-rx'>Radio: Idle</div></section><style>#status .pill:nth-child(3),#status .pill:nth-child(4){display:none}</style>
