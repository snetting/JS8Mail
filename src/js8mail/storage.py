...
class Database:
    ...
    def _migrate(self) -> None:
        ...
        if current < 24:
            self.connection.executescript(
                """
                ALTER TABLE inbox_messages ADD COLUMN read_at_ms INTEGER;
                INSERT INTO schema_migrations(version, applied_at_ms)
                    VALUES (24, strftime('%s','now') * 1000);
                """
            )
        if current < 25:
            # Existing mailbox rows predate read tracking and must not all
        ...
        if current < 26:
            self.connection.execute(
                "ALTER TABLE transmission_transactions ADD COLUMN band TEXT NOT NULL DEFAULT ''"
            )
            self.connection.execute(
                "INSERT INTO schema_migrations(version, applied_at_ms) "
        ...
        if current < 27:
            self.connection.execute(
                "ALTER TABLE inbox_messages ADD COLUMN subject TEXT NOT NULL DEFAULT ''"
            )
            self.connection.execute(
                "INSERT INTO schema_migrations(version, applied_at_ms) "
        ...
        if current < 28:
            self.connection.executescript(
                """
                CREATE TABLE IF NOT EXISTS outgoing_part_progress (
                    message_id TEXT PRIMARY KEY REFERENCES messages(id) ON DELETE CASCADE,
                    total_parts INTEGER NOT NULL CHECK(total_parts BETWEEN 1 AND 255),
                    received_bitmap TEXT NOT NULL DEFAULT '0',
                    updated_at_ms INTEGER NOT NULL
                );
                INSERT INTO schema_migrations(version, applied_at_ms)
    ...
    def begin_transmission_transaction(
        ...
        cursor = self.connection.execute(
            ...
            "created_at_ms, response_timeout_ms, ack_deadline_ms, retry_number, band) "
            "VALUES (?, ?, ?, ?, ?, ?, 'queued', ?, ?, ?, ?, ?)",
            (
                message_id,
                operation,
    ...
    def finish_transmission(self, transaction_id: int) -> None:
        """Anchor one transaction's ACK deadline to the final RF frame."""
        now = utc_now_ms()
        self.connection.execute(
            "UPDATE transmission_transactions SET status=CASE WHEN status='acknowledged' "
            "THEN status ELSE 'awaiting_ack' END, "
            "tx_finished_at_ms=?, ack_deadline_ms=? + response_timeout_ms "
            "WHERE id=? AND status IN ('queued','tx_active','awaiting_ack','acknowledged') "
            "AND tx_finished_at_ms IS NULL",
            (now, now, transaction_id),
        )
        self.connection.commit()

    def get_transmission_transaction(self, transaction_id: int) -> dict[str, Any] | None:
        row = self.connection.execute(
            "SELECT * FROM transmission_transactions WHERE id=?", (transaction_id,)
        ).fetchone()
        return dict(row) if row is not None else None

    def complete_broadcast_transmission(self, transaction_id: int) -> None:
        """A broadcast is sent after RF completion; no ACK is expected."""
        now = utc_now_ms()
        self.connection.execute(
            "UPDATE transmission_transactions SET status='acknowledged', tx_finished_at_ms=?, "
            "ack_deadline_ms=? WHERE id=? AND operation='group_broadcast' "
            "AND status IN ('queued','tx_active','awaiting_ack')",
            (now, now, transaction_id),
        )
        self.connection.commit()

    def expire_unfinished_broadcasts(self, now_ms: int | None = None) -> list[dict[str, Any]]:
        """Stop a lost-PTT broadcast from being automatically retransmitted."""
        now = utc_now_ms() if now_ms is None else int(now_ms)
        rows = self.connection.execute(
            "SELECT * FROM transmission_transactions WHERE operation='group_broadcast' "
            "AND status IN ('queued','tx_active','awaiting_ack') AND created_at_ms <= ?",
            (now - 30 * 60 * 1000,),
        ).fetchall()
        if rows:
    ...
    def expire_transmission_transactions(self, now_ms: int | None = None) -> list[dict[str, Any]]:
        now = utc_now_ms() if now_ms is None else int(now_ms)
        rows = self.connection.execute(
            "SELECT * FROM transmission_transactions WHERE operation != 'group_broadcast' AND "
            "((status='awaiting_ack' AND tx_finished_at_ms IS NOT NULL AND ack_deadline_ms <= ?) "
            "OR (status IN ('queued','tx_active','awaiting_ack') AND tx_finished_at_ms IS NULL "
            "AND created_at_ms <= ?))",
            (now, now - 30 * 60 * 1000),
        ).fetchall()
        if rows:
    ...
    def recent_transmission_transactions(
        self, since_ms: int, band: str | None = None
    ) -> list[dict[str, Any]]:
        """Return recently submitted RF transactions for activity views.

        The JS8Call event stream does not include the human-readable text in
        TX.FRAME events.  The durable transaction row is therefore the
        authoritative local-TX evidence available to the live activity graph.
        Only submitted/active/acknowledged transactions are included; a route
        merely selected by the planner must not look like RF activity.
        """
        query = (
            "SELECT * FROM transmission_transactions "
            "WHERE submitted_at_ms IS NOT NULL AND "
            "COALESCE(tx_finished_at_ms, submitted_at_ms, created_at_ms) >= ?"
        )
        values: list[Any] = [int(since_ms)]
        if band and band.strip():
            query += " AND band = ?"
            values.append(band.strip().lower())
        query += " ORDER BY COALESCE(tx_finished_at_ms, submitted_at_ms, created_at_ms) DESC"
    ...
    def get_message(self, message_id: str) -> dict[str, Any] | None:
        row = self.connection.execute(
            "SELECT * FROM messages WHERE id = ? COLLATE NOCASE", (message_id,)
        ).fetchone()
        return dict(row) if row is not None else None
    ...
    def recent_message_path(
        self, message_id: str, *, max_age_ms: int, now_ms: int | None = None
    ) -> tuple[str, ...]:
        now = utc_now_ms() if now_ms is None else now_ms
        row = self.connection.execute(
            "SELECT path FROM message_paths WHERE message_id=? AND last_attempted_at_ms>=? "
            "ORDER BY last_attempted_at_ms DESC LIMIT 1",
            (message_id, now - max_age_ms),
        ).fetchone()
        return tuple(str(row[0]).split("→")) if row is not None else ()
    ...
    def outgoing_part_bitmap(self, message_id: str, total_parts: int) -> int:
        """Get cumulative receiver-confirmed parts for a local message."""
        if not 1 <= total_parts <= 255:
            raise ValueError("invalid part count")
        row = self.connection.execute(
            "SELECT total_parts, received_bitmap FROM outgoing_part_progress WHERE message_id=?",
            (message_id.lower(),),
        ).fetchone()
        if row is None:
            return 0
        if int(row["total_parts"]) != total_parts:
            raise ValueError("multipart total changed")
        return int(str(row["received_bitmap"]), 16)

    def merge_outgoing_part_receipt(
        self, message_id: str, total_parts: int, received: tuple[int, ...]
    ) -> bool:
        """Persist a PA cumulatively; a late/duplicate bitmap cannot regress it."""
        prior = self.outgoing_part_bitmap(message_id, total_parts)
        if any(not 1 <= number <= total_parts for number in received):
            raise ValueError("invalid received part")
        merged = prior | sum(1 << (number - 1) for number in received)
        self.connection.execute(
            "INSERT INTO outgoing_part_progress(message_id, total_parts, received_bitmap, updated_at_ms) "
            "VALUES (?, ?, ?, ?) ON CONFLICT(message_id) DO UPDATE SET "
            "received_bitmap=excluded.received_bitmap, updated_at_ms=excluded.updated_at_ms",
            (message_id.lower(), total_parts, f"{merged:X}", utc_now_ms()),
        )
        self.connection.commit()
        return merged != prior
    ...
    def upsert_inbox_message(
        ...
        self.connection.execute(
            ...
            "subject=CASE WHEN excluded.subject != '' THEN excluded.subject ELSE inbox_messages.subject END, "
            "read_at_ms=CASE WHEN inbox_messages.body=excluded.body "
            "AND inbox_messages.complete=excluded.complete "
    ...
    def list_inbox(self, group_only: bool = False) -> list[dict[str, Any]]:
        ...
        for row in rows:
            ...
            item["is_read"] = row["read_at_ms"] is not None
    ...
    def mark_inbox_read(self, sender: str, message_id: str) -> None:
        """Mark one local inbox item read; repeating the action is harmless."""
        cursor = self.connection.execute(
            "UPDATE inbox_messages SET read_at_ms = ? WHERE sender = ? AND message_id = ?",
            (utc_now_ms(), sender.upper(), message_id),
        )
        ...
        self.audit(
            "inbox.message_read",
            {"sender": sender.upper(), "message_id": message_id},
        )
