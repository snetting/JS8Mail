...
section{background:white;border:1px solid #d9e0e7;border-radius:10px;padding:1em;margin:1em 0}input,textarea,select{box-sizing:border-box;width:100%;padding:.5em;margin:.25em 0 .7em}textarea{height:110px}button{background:#1769aa;color:#fff;border:0;border-radius:5px;padding:.5em .8em;margin:.2em;cursor:pointer}.danger{background:#a33}.pill{display:inline-block;padding:.3em .6em;border-radius:1em;background:#e8edf2;margin:.2em}.ok{background:#d8f3dc}.warn{background:#fff1c2}.mono{font:12px monospace;white-space:pre-wrap}td,th{text-align:left;border-bottom:1px solid #e4e9ee;padding:.5em;vertical-align:top}details summary{cursor:pointer;padding:.25em 0}details summary::marker{color:#1769aa}
...
async function showStationRoute(call){let s=await api('/api/status');document.querySelector('#route input[name=origin]').value=s.callsign||'';document.querySelector('#route input[name=destination]').value=call;routeQuery=new URLSearchParams({origin:s.callsign||'',destination:call});await refresh()}
async function refreshStations(){let s=await api('/api/stations');document.getElementById('stations').innerHTML=s.length?'<table><tr><th>Callsign</th><th>Age</th><th>SNR</th><th>Evidence</th><th>Action</th></tr>'+s.map(x=>`<tr><td><b>${esc(x.callsign)}</b></td><td>${esc(x.age_seconds)}s ago</td><td>${x.snr==null?'—':esc(x.snr)+' dB'}</td><td>${esc(x.evidence.join(', '))}</td><td><button onclick="useStation('${esc(x.callsign)}')">Compose to</button><button onclick="showStationRoute('${esc(x.callsign)}')">Route</button></td></tr>`).join('')+'</table>':'<p>No recent station evidence.</p>'}
