...
| Emergency halt | `RIG.TX_HALT` | Not in local 2.4 source | 3.0 | Keep as an explicit operator/emergency control only; never invoke it from `TX.FRAME` or normal enhanced delivery. |
...
| TX frame evidence | `TX.FRAME` | Emitted | Emitted | Early frame-preparation evidence only; count it, but do not treat it as completion or issue `RIG.TX_HALT`. |
