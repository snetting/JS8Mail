...
class Handler(BaseHTTPRequestHandler):
    ...
    async def transmit(self, message_id: str, selected_plan: Any | None = None) -> None:
        ...
        if destination.startswith("@"):
            # Group traffic is ordinary JS8Call broadcast mail. Keep the
            # visible version marker on every broadcast so listening JS8Mail
            # clients can recognise the sender without treating it as a CAP
            # request (which could create a group-wide reply storm).
            wire_texts = (format_ordinary_message(destination, str(message["body"]), announce=True),)
        ...
        if action == "group_broadcast":
            self.service.database.record_attempt(
                ...
                target,
                "complete",
                "broadcast handed to JS8Call; no recipient ACK expected",
            )
            self.service.database.transition_message(message_id, MessageState.DELIVERED)
        self.service.database.audit(
