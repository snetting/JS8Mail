...
let stationCache=[];function renderStations(){let q=document.getElementById('station-search').value.trim().toUpperCase();let s=stationCache.filter(x=>!q||x.callsign.includes(q)||x.evidence.join(' ').toUpperCase().includes(q));document.getElementById('stations').innerHTML=s.length?'<table style="table-layout:fixed;width:100%"><colgroup><col style="width:18%"><col style="width:18%"><col style="width:14%"><col style="width:24%"><col style="width:18%"><col style="width:8%"></colgroup><tr><th style="white-space:nowrap">Callsign</th><th style="white-space:nowrap">Age</th><th style="white-space:nowrap">SNR</th><th>Evidence</th><th style="white-space:nowrap">Action</th><th title="JS8Mail capability" style="white-space:nowrap;text-align:center">JS8M</th></tr>'+s.map(x=>`<tr><td style="white-space:nowrap"><b>${esc(x.callsign)}</b></td><td style="white-space:nowrap">${esc(relativeAge(x.age_seconds))}</td><td style="white-space:nowrap">${x.snr==null?'—':esc(x.snr)+' dB'}</td><td>${esc(x.evidence.map(evidenceLabel).join(', '))}</td><td style="white-space:nowrap"><button style="white-space:nowrap" onclick="useStation('${esc(x.callsign)}')">Compose</button></td><td title="${x.js8m?'JS8Mail capable':'Not identified as JS8Mail capable'}" style="white-space:nowrap;text-align:center;color:#16a34a;font-size:1.15em">${x.js8m?'●':''}</td></tr>`).join('')+'</table>':'<p>No matching station evidence.</p>'}async function refreshStations(){stationCache=await api('/api/stations');renderStations()}
