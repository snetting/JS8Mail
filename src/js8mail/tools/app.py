...
function compactOutboxTimelinesV2(){const maxVisible=40;document.querySelectorAll('#messages details .mono').forEach(el=>{delete el.dataset.compacted;let raw=(el.innerHTML||'').split('<br>');if(raw.length<2)return;let entries=raw.map(html=>{let holder=document.createElement('span');holder.innerHTML=html;let time=holder.querySelector('.timeline-time')?.textContent||'';holder.querySelector('.timeline-time')?.remove();let key=holder.textContent.replace(/\\s+/g,' ').trim();let signature=key.replace(/weakest link [0-9.]+/g,'weakest link').replace(/estimated airtime [0-9]+ ms/g,'estimated airtime').replace(/retry(?:ing)? in [0-9]+ minute\\(s\\)/g,'retry backoff').replace(/for up to [0-9]+ seconds?/g,'for bounded response window');return {html,key,signature,time}}),signatures=entries.map(x=>x.signature),rendered=[],i=0;while(i<entries.length){let bestLen=0,bestCount=0,maxLen=Math.min(20,Math.floor((entries.length-i)/2));for(let len=maxLen;len>=1;len--){let signature=signatures.slice(i,i+len).join('\\u0001'),count=1;while(i+(count+1)*len<=entries.length&&signatures.slice(i+count*len,i+(count+1)*len).join('\\u0001')===signature)count++;if(count>=2){bestLen=len;bestCount=count;break}}if(bestCount>=2){let block=entries.slice(i,i+bestLen),times=block.map(x=>x.time).filter(Boolean),summary=block.slice(0,3).map(x=>x.key).join(' · ')+(block.length>3?` · +${block.length-3} events`:'');rendered.push(`<span class='timeline-time'>${esc(times[0]||'')}</span> <span class='pill warn'>Repeated ×${bestCount}</span> ${esc(summary)}${times.length>1?` <span class='timeline-time'>through ${esc(times[times.length-1])}</span>`:''}`);i+=bestLen*bestCount}else{rendered.push(entries[i].html);i++}}if(rendered.length>maxVisible){let omitted=rendered.length-maxVisible;rendered=[`<span class='pill'>Earlier ${omitted} timeline entries hidden; full history remains stored.</span>`,...rendered.slice(-maxVisible)]}if(rendered.length<raw.length||rendered.length<=maxVisible){el.innerHTML=rendered.join('<br>');el.dataset.compacted='1'}})}outboxTimelineObserver.disconnect();outboxTimelineObserver=new MutationObserver(compactOutboxTimelinesV2);outboxTimelineObserver.observe(document.getElementById('messages'),{childList:true,subtree:true});compactOutboxTimelinesV2();
