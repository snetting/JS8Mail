...
</style><h1>JS8Mail</h1><p>Resilient radio mail for reliable offline communication · by <a href='https://www.oh3spn.fi' target=_blank rel=noopener>OH3SPN</a> <button onclick="useStation('OH3SPN')">Compose to OH3SPN</button></p><section><div id=status>Loading…</div><div id=radio-state class='pill radio-rx'>Radio: Idle</div></section><style>#status .pill:nth-child(3),#status .pill:nth-child(4){display:none}</style>
        ...
        while True:
            try:
                ...
                async def handle(event: NormalizedEvent) -> None:
                    ...
                    elif event.event_type in {"RX.ACTIVITY", "RX.BAND_ACTIVITY"}:
                        status["radio_activity"] = "RX"
                    elif event.event_type.startswith("RX"):
                        status["radio_activity"] = "Decode"
                    else:
                        status["radio_activity"] = "API"
