...
document.querySelector('.version-button')?.replaceChildren(document.createTextNode('v0.0.7 · Updates'));
const showReleaseInfo=showVersionInfo;showVersionInfo=()=>{showReleaseInfo();let title=document.querySelector('#version-modal h2');if(title)title.textContent='JS8Mail v0.0.7';let list=document.querySelector('#version-modal ul');if(list)list.innerHTML='<li>Reassembles standard and JS8Mail activity frames, including bare multiframe receipts.</li><li>Preserves partial multipart mail and correlates selective acknowledgements and final delivery receipts.</li><li>Serializes TX trains, protects RX response windows, and retains routes across busy-radio deferrals.</li><li>Improves inbox selection, stable compact previews, protocol labels, and active-band RF evidence.</li><li>Opportunistic mode actively learns JS8M capability from marked first contact, while Standard mode remains a complete one-message opt-out.</li><li>Group broadcasts now omit the JS8Mail marker to save airtime; capability records remain valid for seven days and refresh with valid JS8M evidence.</li><li>Thanks to the operators who helped with local two-station and on-air testing and reported the edge cases behind this release.</li>'};
</script>"""
