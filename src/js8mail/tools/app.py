...
<section><h2>Inbox</h2><div id=inbox>Loading…</div></section><section><h2>Outbox</h2><div id=messages>Loading…</div></section><section><h2>Groups and bulletins</h2><p>Groups are observed and available here; automatic forwarding remains opt-in.</p><div id=groups>Loading…</div></section>
...
function relativeAge(seconds){seconds=Math.max(0,Number(seconds)||0);if(seconds<60)return `${Math.round(seconds)}s ago`;if(seconds<3600)return `${Math.floor(seconds/60)}m ago`;if(seconds<86400)return `${Math.floor(seconds/3600)}h ago`;return `${Math.floor(seconds/86400)}d ago`}function evidenceLabel(value){return value==='direct'?'Direct':value==='reported_target'?'Remote':value==='remote_report'?'Via remote report':value}async function showStationGraph(call){try{let s=await api('/api/status'),r=await api('/api/route?origin='+encodeURIComponent(s.callsign||'')+'&destination='+encodeURIComponent(call));document.getElementById('graph-result').innerHTML=`<p><b>${esc(s.callsign||'')} → ${esc(call)}</b></p><p>${esc(r.explanation)}</p><p class=mono>${esc(r.path.join(' → '))}</p>`}catch(e){document.getElementById('graph-result').textContent=e}}
        ...
        try:
            ...
            if len(group_parts) == 4 and group_parts[:2] == ["api", "groups"]:
                ...
                self.service.database.set_group_subscription(
                    group_parts[2], group_parts[3] == "subscribe"
                )
