...
</style><h1>JS8Mail</h1><p>Resilient radio mail for reliable offline communication · by OH3SPN <button onclick="useStation('OH3SPN')">Compose to OH3SPN</button></p><section><div id=status>Loading…</div></section><style>#status .pill:nth-child(3),#status .pill:nth-child(4){display:none}</style>
            ...
            try:
                ...
                async def handle(event: NormalizedEvent) -> None:
                    if event.event_type.startswith("TX"):
                        ...
                        status["radio_activity"] = "RX / decode"
                    else:
